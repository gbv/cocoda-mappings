...
<?php
$html = (file_exists('../cocoda/app/mappings.html')
      ? file_get_contents('../cocoda/app/mappings.html')
      : file_get_contents('http://coli-conc.gbv.de/cocoda/app/mappings.html')) ?? '';
if (preg_match_all('!src=["\']?([^"\'>]+)!', $html, $match)) {
  foreach ($match[1] as $src) {
      echo "<script type='text/javascript' src='../cocoda/app/$src'></script>";
  }
}
if (preg_match_all('!link href=["\']?([^"\'> ]+)!', $html, $match)) {
  foreach ($match[1] as $src) {
    if (strpos($src, "bootstrap") === false) {
      echo "<link href='../cocoda/app/$src' rel='stylesheet'>";
    }
  }
}
